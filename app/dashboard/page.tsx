...
import { auth, rtdb } from "@/lib/firebase"; // Only need rtdb and auth
...
export default function DashboardPage() {
    ...
    const [wishlistCount, setWishlistCount] = useState(0); // Placeholder if wishlist logic isn't ready
    ...
    // 2. Fetch Cart Count (Realtime DB)
    useEffect(() => {
        if (!user) return;
    ...
    // 3. Fetch Orders Count (Realtime DB - FIX)
    useEffect(() => {
        if (!user) return;
        // ✅ CORRECTED PATH: Fetching from Realtime Database 'orders/{uid}'
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
                    ...
                    <aside className="bg-white p-6 shadow-lg rounded-xl border-gray-400space-y-6 h-fit lg:sticky lg:top-28">
                        ...
                        <nav className="space-y-2 text-gray-700">
                            <Link href="/profile" className="block p-3 rounded-lg bg-orange-100 text-orange-600 font-semibold">Dashboard</Link>
                            <Link href="/my-orders" className="block p-3 rounded-lg hover:bg-gray-100 transition">My Orders</Link>
                        ...
                        <div className="bg-white border-gray-400shadow-sm p-6 rounded-xl">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-700">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-lg">{new Date(user.metadata.creationTime).toLocaleDateString()}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 uppercase font-bold">Account Status</p>
                                    <p className="text-green-600 font-bold bg-green-100 inline-block px-2 py-1 rounded text-sm mt-1">Active</p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 uppercase font-bold">Login Method</p>
